...
from app.ml.inference import predict_ocsvm
import os
import json
...
@router.get("/ml/ocsvm/status")
def get_ocsvm_status():
    """
    ...
    try:
        ...
        return {
            "model_loaded": True,
            "model_version": metadata.get("model_version", "ocsvm_v1"),
            "training_timestamp": metadata.get("training_timestamp"),
            "feature_count": len(metadata.get("features", []))
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to read OCSVM metadata: {e}")

@router.post("/ml/ocsvm/predict")
def predict_patient_anomaly(payload: dict, db: Session = Depends(get_db)):
    """
    Retrieves a patient's features from the database, runs OCSVM prediction, and returns the output.
    """
    ...
    try:
        pred_res = predict_ocsvm(features_dict)
        return {
            "patient_id": patient_id,
            "model_version": "ocsvm_v1",
            "ocsvm_decision": pred_res["ocsvm_decision"],
            "ocsvm_raw_score": pred_res["ocsvm_raw_score"],
            "ocsvm_anomaly_score": pred_res["ocsvm_anomaly_score"],
